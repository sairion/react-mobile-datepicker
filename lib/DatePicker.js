...
class DatePicker extends Component {
  ...
  render() {
    ...
    const themeClassName =
      ["default", "dark", "ios", "android", "android-dark"].indexOf(theme) === -1 ? "default" : theme

    const dataConfigList = this.normalizeDateConfig(dateConfig)

    return (
      <div className={`datepicker ${themeClassName} ${className}`}>
        {/* {showHeader && <div className="datepicker-header">{customHeader || convertDate(value, headerFormat)}</div>} */}
        {/* {showCaption && (
        ...
        {/* {showFooter && (
          <div className="datepicker-navbar">
            <a className="datepicker-navbar-btn" onClick={this.handleFinishBtnClick}>
              {confirmText}
            </a>
